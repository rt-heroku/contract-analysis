...
```json
// devDependencies
"@types/uuid": "^9.0.0"  // Must match uuid major version
```

**Command:**
```bash
npm install --save-dev @types/uuid@9.0.0
```
...
- **Version must match:** @types/uuid@9.x for uuid@9.x

**⚠️ Important:** Initially tried @types/uuid@10.0.0 which caused build failures. The @types version MUST match the major version of the uuid package.
...
**Commits:**
1. **`d633cd5`** - `fix(upload): Fix ES Module error by downgrading uuid to v9`
2. **`fd709b8`** - `docs(upload): Add comprehensive documentation for uuid ES Module fix`
3. **`cc00da5`** - `fix(deps): Correct @types/uuid version to match uuid@9.0.1`

**Branch:** `main`  
**Author:** [Auto-generated by AI Assistant]  
**Date:** December 19, 2025

**Note:** The third commit fixes a version mismatch where @types/uuid@10.0.0 was incompatible with uuid@9.0.1, causing Heroku build failures.
