-- Add Role Management and Menu Management to Admin Panel submenu
...
DECLARE
  ...
  admin_role_id INT;
BEGIN
  -- Get Admin Panel menu ID
  SELECT id INTO admin_menu_id FROM "menu_items" WHERE title = 'Admin Panel' AND parent_id IS NULL;
  
  -- Get admin role ID
  SELECT id INTO admin_role_id FROM "roles" WHERE name = 'admin';
  
  IF admin_menu_id IS NOT NULL THEN
    RAISE NOTICE 'Found Admin Panel menu with ID: %', admin_menu_id;
    ...
    VALUES (admin_menu_id, 'Roles', 'Shield', '/admin/roles', FALSE, 2, TRUE, NOW(), NOW())
    ...
    INSERT INTO "menu_items" (parent_id, title, icon, route, is_external, order_index, is_active, created_at, updated_at)
    VALUES (admin_menu_id, 'Menu', 'Menu', '/admin/menu', FALSE, 3, TRUE, NOW(), NOW())
    ON CONFLICT DO NOTHING
    ...
    IF role_mgmt_id IS NOT NULL AND admin_role_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Role Management menu (ID: %) added and assigned to admin role', role_mgmt_id;
    END IF;
    ...
    IF menu_mgmt_id IS NOT NULL AND admin_role_id IS NOT NULL THEN
      ...
      RAISE NOTICE 'Menu Management menu (ID: %) added and assigned to admin role', menu_mgmt_id;
    END IF;
  ELSE
    RAISE NOTICE 'Admin Panel menu not found!';
  END IF;
END $$;
...
WHERE p.title = 'Admin Panel' OR (m.title = 'Admin Panel' AND m.parent_id IS NULL)
GROUP BY m.id, p.title, m.title, m.route, m.order_index, m.is_active
