...
export const RadioGroup: React.FC<RadioGroupProps> & { craft?: any } = ({
  ...
  return (
    ...
    >
      ...
      <div className={`flex ${orientation === 'vertical' ? 'flex-col space-y-2' : 'flex-row flex-wrap gap-4'}`}>
        {options.map((option, index) => (
          <label key={index} className="flex items-center gap-2 cursor-pointer">
            <input
              ...
              required={required}
              className="w-4 h-4 text-primary-600 border-gray-300 dark:border-gray-600 focus:ring-2 focus:ring-primary-500"
            />
